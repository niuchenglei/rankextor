...
from base.ranker import NeuralRanker
from model.eval.parameter import ModelParameter
...
class RankNet(NeuralRanker):
    ...
    def inner_train(self, batch_pred, batch_label, batch_weight, **kwargs):
        ...
        print(torch.unsqueeze(batch_pred, dim=2).shape)
        print(batch_s_ij.shape)
        print(batch_weight.shape)
        print("-------")
        batch_mask = ((torch.unsqueeze(batch_weight, dim=2) - torch.unsqueeze(batch_weight, dim=1)) == 0).long()

        batch_p_ij = 1.0 / (torch.exp(-self.sigma * batch_s_ij) + 1.0) * batch_mask
        ...
        batch_w = (torch.unsqueeze(batch_weight, dim=2) + torch.unsqueeze(batch_weight, dim=1))/2
        ...
        batch_loss = F.binary_cross_entropy(input=torch.triu(batch_p_ij, diagonal=1), target=torch.triu(batch_std_p_ij, diagonal=1), weight=torch.triu(batch_w, diagonal=1), reduction='mean')
